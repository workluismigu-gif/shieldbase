    ...
    <div className="max-w-3xl space-y-8">
      ...
      <div className="bg-white rounded-2xl border border-gray-200 p-6">
        <div className="flex items-center gap-3 mb-4">
          ...
          <h2 className="text-lg font-semibold text-gray-900">Install Prowler</h2>
        </div>
        <p className="text-sm text-gray-500 mb-3">Prowler is a free, open-source security tool. Requires Python 3.9+.</p>
        <div className="bg-gray-900 rounded-lg p-4 flex justify-between items-center">
          ...
          <button onClick={() => copyToClipboard(PROWLER_INSTALL, "install")}
            className="text-gray-400 hover:text-white text-xs ml-4 flex-shrink-0">
            {copied === "install" ? "✓ Copied" : "Copy"}
          </button>
        </div>
        <p className="text-xs text-gray-400 mt-2">
          Or install via brew: <code className="bg-gray-100 px-1 rounded">brew install prowler</code>
        </p>
      </div>
      ...
      <div className="bg-white rounded-2xl border border-gray-200 p-6">
        <div className="flex items-center gap-3 mb-4">
          ...
          <h2 className="text-lg font-semibold text-gray-900">Configure AWS Credentials</h2>
        </div>
      ...
      <div className="bg-white rounded-2xl border border-gray-200 p-6">
        <div className="flex items-center gap-3 mb-4">
          ...
          <h2 className="text-lg font-semibold text-gray-900">Run the SOC 2 Scan</h2>
        </div>
        <p className="text-sm text-gray-500 mb-3">
          Run this command. It takes 5–15 minutes depending on how many resources you have. The output JSON file is saved to <code className="bg-gray-100 px-1 rounded">/tmp/shieldbase-scan/</code>
        </p>
        <div className="bg-gray-900 rounded-lg p-4 flex justify-between items-start">
        ...
        <p className="text-xs text-gray-400 mt-2">
          This maps checks to the SOC 2 framework. Look for a <code className="bg-gray-100 px-1 rounded">.json</code> file in the output directory when it finishes.
        </p>
      </div>
      ...
      <div className="bg-white rounded-2xl border border-gray-200 p-6">
        <div className="flex items-center gap-3 mb-4">
          ...
          <h2 className="text-lg font-semibold text-gray-900">Upload Your Results</h2>
        </div>
        ...
        {status === "success" && result ? (
          <div className="bg-green-50 border border-green-200 rounded-xl p-6 text-center">
            ...
            <a href="/dashboard" className="inline-block bg-blue-600 hover:bg-blue-700 text-white px-6 py-2.5 rounded-lg font-semibold text-sm transition">
              View Dashboard →
            </a>
          </div>
        ) : (
          <>
            <div className={`border-2 border-dashed rounded-xl p-8 text-center transition ${file ? "border-blue-400 bg-blue-50" : "border-gray-200 hover:border-gray-300"}`}
            ...
            <button onClick={handleUpload} disabled={!file || status === "uploading"}
              className="w-full mt-4 bg-blue-600 hover:bg-blue-700 disabled:opacity-50 text-white py-3 rounded-lg font-semibold transition">
      ...
      <div className="bg-gray-50 rounded-2xl border border-gray-100 p-6">
        <h3 className="font-semibold text-gray-800 mb-3">Common questions</h3>
        <div className="space-y-3 text-sm">
          <div>
            <p className="font-medium text-gray-700">Is this safe? Does Prowler modify anything?</p>
            <p className="text-gray-500 mt-0.5">Prowler is 100% read-only. It only reads configuration data — it never changes, deletes, or creates anything in your AWS account.</p>
          </div>
          <div>
            <p className="font-medium text-gray-700">What does ShieldBase do with my scan data?</p>
            <p className="text-gray-500 mt-0.5">We store it in your private Supabase database to populate your dashboard. It&apos;s never shared with third parties.</p>
          </div>
          <div>
            <p className="font-medium text-gray-700">The scan is taking a long time</p>
            <p className="text-gray-500 mt-0.5">Normal — Prowler runs hundreds of checks. Large accounts (100+ resources) can take 20–30 minutes. Let it finish.</p>
          </div>
        </div>
      </div>
    </div>
