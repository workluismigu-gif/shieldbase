    ...
    <div className="min-h-screen bg-gray-50">
        ...
        <div className="max-w-4xl mx-auto px-6 py-4 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <div className="text-xs text-gray-400">Trust Center</div>
            </div>
          </div>
          <div className="flex items-center gap-3">
            <a href={company.website} target="_blank" rel="noopener noreferrer"
              className="text-sm text-gray-500 hover:text-gray-700 transition hidden md:block">
      ...
      <div className="max-w-4xl mx-auto px-6 py-10 space-y-10">
        ...
        <div className="text-center">
          <h1 className="text-3xl font-black text-gray-900 mb-3">{company.name}</h1>
          <p className="text-gray-500 text-lg max-w-2xl mx-auto">{company.description}</p>
          <p className="text-xs text-gray-400 mt-4">Last updated: {company.lastUpdated} · Powered by ShieldBase</p>
        </div>

        {/* Overall score */}
        <div className="bg-white rounded-2xl border border-gray-200 p-8 text-center">
          <div className="text-6xl font-black text-blue-600 mb-2">{pct}%</div>
          <div className="text-lg font-semibold text-gray-700 mb-1">Compliance Controls Passing</div>
          <div className="text-sm text-gray-400">{company.controls.compliant} of {company.controls.total} SOC 2 controls verified</div>
          <div className="w-full bg-gray-100 rounded-full h-3 max-w-sm mx-auto mt-4">
        ...
        {/* Certifications */}
        <div>
          <h2 className="text-xl font-bold text-gray-900 mb-4">Certifications & Frameworks</h2>
          <div className="grid md:grid-cols-3 gap-4">
            {company.frameworks.map((f, i) => (
              <div key={i} className={`bg-white rounded-2xl border p-6 text-center ${f.status === "certified" ? "border-green-200 bg-green-50" : "border-gray-200"}`}>
                <div className="text-3xl mb-2">{f.icon}</div>
                <div className="font-bold text-gray-900">{f.name}</div>
                <div className={`text-sm mt-1 font-medium capitalize ${f.status === "certified" ? "text-green-600" : f.status === "in_progress" ? "text-blue-600" : "text-gray-400"}`}>
        ...
        {/* Control categories */}
        <div>
          <h2 className="text-xl font-bold text-gray-900 mb-4">Security Controls by Category</h2>
            ...
            {company.controls.categories.map((cat, i) => (
              <div key={i} className="bg-white rounded-2xl border border-gray-200 p-5">
                <div className="flex items-center justify-between mb-3">
                  ...
                  <span className="text-lg font-bold text-blue-600">{cat.score}%</span>
                </div>
        ...
        {/* Policies */}
        <div>
          <h2 className="text-xl font-bold text-gray-900 mb-4">Security Policies</h2>
          <div className="bg-white rounded-2xl border border-gray-200 overflow-hidden">
            {company.policies.map((policy, i) => (
              <div key={i} className={`flex items-center gap-3 px-5 py-3.5 ${i < company.policies.length - 1 ? "border-b border-gray-100" : ""}`}>
                <span className="text-green-500 font-bold">✓</span>
                <span className="text-sm text-gray-700">{policy}</span>
                <span className="ml-auto text-xs text-gray-400 bg-green-100 text-green-700 px-2 py-0.5 rounded-full">Approved</span>
              </div>
            ))}
          </div>
        </div>
        ...
        <div>
          <h2 className="text-xl font-bold text-gray-900 mb-4">Security Tools & Integrations</h2>
          <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
            {company.integrations.map((int, i) => (
              <div key={i} className="bg-white rounded-xl border border-gray-200 p-4 flex items-center gap-3">
                <div className="w-9 h-9 bg-gray-100 rounded-lg flex items-center justify-center text-xl">{int.logo}</div>
                <div>
                  <div className="text-sm font-semibold text-gray-800">{int.name}</div>
                  <div className="text-xs text-gray-400">{int.description}</div>
                </div>
              </div>
            ))}
          </div>
        ...
        {/* FAQ */}
        <div>
          <h2 className="text-xl font-bold text-gray-900 mb-4">Security FAQ</h2>
        ...
        <div className="bg-navy rounded-2xl p-8 text-center">
          ...
          <p className="text-gray-400 text-sm mb-6">Request our SOC 2 Type I report, penetration test summary, or security questionnaire under NDA.</p>
